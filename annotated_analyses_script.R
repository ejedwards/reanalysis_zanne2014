...
#  analyses verbatim from this script (in series) takes a very long time.  We executed 
#  analyses that use the function corHMM::corDISC() on the OSCAR computer cluster at 
...
#   - objective 1.  Ascertain whether Zanne et al. analyzed vessel cross-sectional area or vessel diameter
...
# adjust and cross-check data
	...
	# format climate data
	clim$species <- stringr::str_replace_all(clim$species, " ", "_")
	...
	woody$species <- stringr::str_replace_all(woody$gs, " ", "_")
	...
	vessel <- data.frame(species=vessel$Binomial, A=vessel$A, D=2*sqrt(vessel$A/pi), row.names=vessel$Binomial)
		# Note, vessel$A is the x-sectional area data; vessel$D is x-sectional area transformed into diameter
...
# analyze results II: get pathways, state lability, and persistence times
	...
	#   close to the numbers of Zanne et al.'s [we: (88.76, 0, 11.24) vs. Zanne et al.: (82.7, 0, 17.3)]
	...
	#   Zanne et al.'s data files vs. 860 reported in their paper). We believe this must be due to their 
	#	name matching procedure, which was not well enough described in supplemental methods for us to
	#	attempt to re-create it (and no scripts were provided).
...
# adjust and cross-check data
	...
	# format climate data
	clim$Spec <- stringr::str_replace_all(clim$species, " ", "_")
	clim$Freeze.tmin.lo[clim$Freeze.tmin.lo=="FreezingExposed"] <- 1
	clim$Freeze.tmin.lo[clim$Freeze.tmin.lo=="FreezingUnexposed"] <- 0
	clim$climate <- as.numeric(clim$Freeze.tmin.lo)
	...
	for(result in c("corDISC_NULL_repeat_asterids_woody", "corDISC_NULL_repeat_rosids_woody", "corDISC_NULL_repeat_magnolids_woody", "corDISC_NULL_repeat_monocots_woody", "corDISC_NULL_repeat_resttree_woody")) 
	{
		...
		simult			#  0   %
		climfirst		# 43.5887 %
		#  this is much closer to 58%, 0%, and 42%, respectively, in the paper, so we assume this is the approach followed.
...
# adjust and cross-check data
	...
	woody <- woody[woody$Spec %in% tree$tip.label,] # throw out species not in the tree
	woody$dat <- 0
	woody$dat[woody$woodiness == "H"] <- 1  # score 1 for herbaceous taxa
...
#  save objects needed for analysis: the trees and the data and rate_mat
	...
	##  running the analyses this way takes a long time; you may want to split up the loops 
...
corHMM_result_ratematall_resttree_", inputlist[i], ".Rdata", sep="")) #save analysis
	...
	##  the set of analyses using no index matrix so simultaneous changes are not allowed
	for(i in 1:length(inputlist)) {
		# get data objects
		data <- get(inputlist[i])
		# select rate mat
...
corHMM_result_NULL_resttree_", inputlist[i], ".Rdata", sep="")) #save analysis
	...
	#########################
	#  task 3.5.  summarize results into a figure
	#########################
	rm(list=ls())
	source('R_source_custom_functions.R')
	res <- read.table("results_ClimateScoringSensitivity.txt", header=TRUE, stringsAsFactors=FALSE, sep="\t")

	## select the n=10, median result for mentioning.
	good <- res$groom=="groomed" & res$perc=="percmed" & res$minclimdat==10
	res_ok <- res[good,]
	res_ok <- res_ok[,c(12, 4:8, 28:30, 33:37)]
	res_ok


	pdf("./figures/Figure_ClimateScoringSensitivity.pdf", width=8.9/2.54, height=8.9/2.54, fonts="sans")
	
#  set-up plotting area and plot
...
#	bgcol <- "#E1E1E1"
	...
	axis(side=1, at=seq(0, 100, 20), labels=paste(seq(0, 100, 20), "%", sep=" "),
		lwd.ticks=0.5, cex.axis=simulcex, padj=-3, tck=-0.02)
	axis(side=2, at=seq(0, 100, 20), labels=paste(seq(0, 100, 20), "%", sep=" "),
...
# add pathways layout
	...
	mtext(side=1, line=0.5, "% contribution climate-first pathway", cex=0.6)
	mtext(side=2, line=0.8, "% contribution trait-first pathway", cex=0.6)

	mtext(line= mtextline-0.15, side=1, adj=0.15, text="% contribution simultaneous pathway: ", col=simultcol, cex=simulcex)
	mtext(line= mtextline-0.12, side=1, adj=1, text="0 %", col=simultcol, cex=simulcex)
	mtext(line= mtextline-0.12, side=1, adj=0.8, text="20 %", col=simultcol, cex=simulcex)
	mtext(line= mtextline-0.12, side=1, adj=0.6, text="40 %", col=simultcol, cex=simulcex)
	mtext(line= mtextline+0.15, side=2, adj=1, text="0 %", col=simultcol, cex=simulcex)
	mtext(line= mtextline+0.15, side=2, adj=0.8, text="20 %", col=simultcol, cex=simulcex)
	mtext(line= mtextline+0.15, side=2, adj=0.6, text="40 %", col=simultcol, cex=simulcex)
	
	segments(x0=0, x1=100, y0=100, y1=0, lty=3, col=simultcol)
	segments(x0=0, x1=80, y0=80, y1=0, lty=2, col=simultcol)
	segments(x0=0, x1=60, y0=60, y1=0, lty=1, col=simultcol)
...
### add star for "repeated" analysis based on their climate data; include in hull

	col <- phenol_col
	rep <- c(43.07, 39.28) # our result based on what zanne did (with their climate data)
	data <- res[res$trait=="phenol", ]
	with(data, points(traitfirst ~ climfirst, pch=19, col=col, cex= cexdata))
	points(x=rep[1], y=rep[2], pch="o", cex=1.666, col=col)	
	hull <- chull(x=c(data$climfirst, rep[1]), y=c(data$traitfirst, rep[2]))
	with(data, polygon(x=c(data$climfirst, rep[1])[hull], y=c(data$traitfirst, rep[2])[hull], border=col, col=makeTransparent(col)))
	
	col <- woody_col
	rep <- c(43.6, 56.4) # our result based on what zanne did (with their climate data)
	...
	col <- vessel_col
	rep <- c(44.9, 55.1) # our result based on what zanne did (with their climate data, with D, not A)
	...
	bgcol <- makeTransparent("white", 200)
	textcex <- 0.54
	plotrix::boxed.labels(x=xtext_trait, y=ytext_trait+2, "Trait-first scenario", cex= textcex, font=3, bg= bgcol, border=NA)
	plotrix::boxed.labels(x=xtext_trait, y=ytext_trait-2, "\"far more\" likely", cex= textcex, font=3, bg= bgcol, border=NA)
	plotrix::boxed.labels(x=xtext_clim, y=ytext_clim+2, "Climate-first scenario", cex= textcex, font=3, bg= bgcol, border=NA)
	plotrix::boxed.labels(x=xtext_clim, y=ytext_clim-2, "\"far more\" likely", cex= textcex, font=3, bg= bgcol, border=NA)
	plotrix::boxed.labels(x=xtext_simul-4, y=ytext_simul+2, "Simultaneous scenario", cex= textcex, font=3, bg= bgcol, border=NA)
	plotrix::boxed.labels(x=xtext_simul-7, y=ytext_simul-2, "\"far more\" likely", cex= textcex, font=3, bg= bgcol, border=NA)


# add zanne result (z) and our re-analysis thereof (o)
	cexdot <- 1.666


	col <- phenol_col
	points(x=43.1, y=39.3, cex=cexdot, pch="o", col=col)	
	points(x=48.7, y=36.7, cex=cexdot, pch="z", col= col)

	col <- woody_col
	points(x=43.6, y=56.4, cex=cexdot, pch="o", col=col)	
	points(x=42.0, y=58.0, cex=cexdot, pch="z", col= col)

	col <- vessel_col
	points(x=44.9, y=55.1, cex=cexdot, pch="o", col=col)	
	points(x=17.3, y=82.7, cex=cexdot, pch="z", col= col)


# add legend
	base <- 45
	textcex <- 0.6
	text(x=base, y=101, "Z", cex=0.666, pos=4, font=2)
	text(x=base+3.9, y=101, ": Point estimates by Zanne et al.", cex=textcex, pos=4)
	text(x=base, y=97, "O", cex=0.666, pos=4, font=2)
	text(x=base+3.9, y=97, ": Point estimates re-analysis", cex=textcex, pos=4)

	text(x=base, y=93, "Estimate range after data cleaning:", cex=textcex, pos=4)

	x <- c(base+4, base+9, base+9, base+4)
	col <- phenol_col
	y <- c(90, 90, 86, 86)
	points(x, y, pch=19, cex=0.75, col=col)
	polygon(x=x, y=y, border=col, col=makeTransparent(col))
	text(x=base+10, y=mean(y), ": Leaf phenology data", cex=textcex, pos=4)	

	col <- vessel_col
	y <- y-6.5
	points(x, y, pch=19, cex=0.75, col=col)
	polygon(x=x, y=y, border=col, col=makeTransparent(col))
	text(x=base+10, y=mean(y), ": Conduit diameter data", cex=textcex, pos=4)	

	col <- woody_col
	y <- y-6.5
	points(x, y, pch=19, cex=0.75, col=col)
	polygon(x=x, y=y, border=col, col=makeTransparent(col))
	text(x=base+10, y=mean(y), ": Woodiness data", cex=textcex, pos=4)	

	polygon(x=c(base, base, 103.5, 103.5), y=c(71, 103.5, 103.5, 71))
...
par(mfrow=c(1,2))
	...
	pdf(width=8.9*2/2.54, height=8.9/2.54, file="./figures/Figure_simulations.pdf")
		par(mfrow=c(1,2), mar=c(1.5,1.5,0.5, 0.5))
		...
		plot(NULL, bty="o",
			...
			type="n", xaxt="n", yaxt="n")
		axis(side=1, at=seq(0, 100, 20), labels=paste(seq(0, 100, 20), "%", sep=" "),
			lwd.ticks=0.5, cex.axis=simulcex, padj=-3.9, tck=-0.02)
		mtext("True % Trait-first pathway counted from 3x biased simulated history", cex=simulcex+0.1, side=1, line=0.3)
		axis(side=2, at=seq(0, 100, 20), labels=paste(seq(0, 100, 20), "%", sep=" "),
			lwd.ticks=0.5, cex.axis=simulcex, padj=2.9, tck=-0.02)
		mtext("% Trait-first pathway inferred from 3x biased simulated history", cex=simulcex+0.1, side=2, line=0.7)
		x <- pathways_biased_true[,1]
		y <- pathways_biased_inf[,1]
		col <- rep("grey20", 200)
		col[pathways_biased_inf[,1]<56] <- "red"
		points(x, y, col=col, pch=19, cex=0.666)
		mtext("Truth: trait-first 3x more likely", cex=0.6, line=-1, adj=0.1, col="red")
		mtext("(no simultaneous changes)", cex=0.6, line=-1.6, adj=0.1, col="red")
		mtext(" Red dots: incorrectly inferred no preference for trait-first", cex=0.6, col="red", side=1, line=-2.6, adj=0)
		mtext(" (following Zanne et al.\'s interpretation of a >12% difference)", cex=0.6, col="red", side=1, line=-2, adj=0)
		mtext(paste(" Thus,", sum(col=="red"), "of", length(col), "analyses are qualitatively incorrect"), cex=0.6, col="red", side=1, line=-1, adj=0)
		...
		axis(side=1, at=seq(0, 100, 20), labels=paste(seq(0, 100, 20), "%", sep=" "),
			lwd.ticks=0.5, cex.axis=simulcex, padj=-3.9, tck=-0.02)
		mtext("True % Trait-first pathway counted from eq-rates simulated history", cex=simulcex+0.1, side=1, line=0.3)
		axis(side=2, at=seq(0, 100, 20), labels=paste(seq(0, 100, 20), "%", sep=" "),
			lwd.ticks=0.5, cex.axis=simulcex, padj=2.9, tck=-0.02)
		mtext("% Trait-first pathway inferred from eq-rates simulated history", cex=simulcex+0.1, side=2, line=0.7)
		x <- pathways_true[,1]
		y <- pathways_inf[,1]
		col <- rep("grey20", 200)
		col[pathways_inf[,1]>56 | pathways_inf[,1]<44] <-"red"
		points(x, y, col=col, pch=19, cex=0.666)
		mtext("Truth: both pathways equally likely ", cex=0.6, line=-1, adj=0.1, col="red")
		mtext("(no simultaneous changes)", cex=0.6, line=-1.6, adj=0.1, col="red")
		mtext(" Red dots: results indicating a trajectory to be \"far more\" likely ", cex=0.6, col="red", side=1, line=-2.6, adj=0)
		mtext(" (following Zanne et al.\'s interpretation of a >12% difference)", cex=0.6, col="red", side=1, line=-2, adj=0)
		mtext(paste(" Thus,", sum(col=="red"), "of", length(col), "analyses are qualitatively incorrect"), cex=0.6, col="red", side=1, line=-1, adj=0)

dev.off()

	# conclusion: see Figure_simulations.pdf:  the method performs qualitatively fairly well when there is an underlying, much more likely trajectory (left panel). However, when both trajectories are equally likely, the method typically (in 154 out of 200 simulated histories) infers a higher contribution for a trajectory nevertheless (i.e., false positive).
